...
export default function ExpenseCategoriesPage() {
  ...
  const isServiceWorkspace = pathname?.startsWith("/service") ?? false
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault()
    if (isServiceWorkspace && readOnly) {
  ...
  const handleEdit = (category: ExpenseCategory) => {
    if (isServiceWorkspace && readOnly) return
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-8">
            ...
            <div className="flex items-center justify-between">
              ...
              {!readOnly && (
                <button
                  onClick={() => guardWriteAction(() => setShowCreateModal(true))}
          ...
          {isServiceWorkspace && readOnly && (
            <ServiceReadOnlyNotice scope="expenses" className="mb-6" />
          )}
          ...
          {/* Edit Modal */}
          {showEditModal && editingCategory && !readOnly && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
