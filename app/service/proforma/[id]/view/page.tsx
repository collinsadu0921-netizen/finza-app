...
export default function ProformaViewPage() {
  ...
  const handleWhatsApp = async () => {
    try {
      setSending(true)
      const phone =
        proforma?.customers?.whatsapp_phone?.trim() || proforma?.customers?.phone?.trim() || null
      if (!phone) {
        setToast({ message: "Customer phone number is missing.", type: "error" })
        return
      }
      const tok = await ensureSentAndGetPublicToken()
      const link = buildClientUrl(tok)
      const message = `Hi${proforma?.customers?.name ? ` ${proforma.customers.name}` : ""},\n\nPlease review and accept your proforma invoice${proforma?.proforma_number ? ` ${proforma.proforma_number}` : ""}.\n\n${link}`
  ...
  const handleEmail = async () => {
    ...
    if (!proforma.customers?.email?.trim()) {
      setToast({ message: "Customer email is missing.", type: "error" })
      return
    }
    try {
      setSending(true)
  ...
  const formatProformaMoney = (amount: number | null | undefined) =>
    formatMoneyForContext(amount, proforma)
                    ...
                    {items.map((item, index) => (
                      <tr key={item.id || index} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-right text-slate-600 tabular-nums">
                          {formatProformaMoney(Number(item.unit_price || 0))}
                        </td>
                        <td className="px-4 py-3 text-right text-slate-600 tabular-nums">
                          {Number(item.discount_amount || 0) > 0
                            ? formatProformaMoney(Number(item.discount_amount))
                            : "—"}
                        </td>
                        <td className="px-4 py-3 text-right font-medium text-slate-800 tabular-nums">
                          {formatProformaMoney(Number(item.line_subtotal || 0))}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div className="w-72 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-slate-800 tabular-nums">
                    {formatProformaMoney(Number(proforma.subtotal))}
                  </span>
                </div>
                {totalDiscount > 0 && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-rose-600 tabular-nums">
                      {formatProformaMoney(-Math.abs(totalDiscount))}
                    </span>
                  </div>
                )}
                {Number(proforma.total_tax) > 0 && (
                  <>
                      ...
                      Object.entries(allTaxLines)
                        ...
                        .map(([code, amount]) => (
                          <div key={code} className="flex justify-between text-sm text-slate-500">
                            <span>{code}:</span>
                            <span className="tabular-nums">{formatProformaMoney(Number(amount))}</span>
                          </div>
                        ))
                    ) : (
                      <>
                        {taxBreakdown.nhil > 0 && (
                          <div className="flex justify-between text-sm text-slate-500">
                            <span>NHIL:</span>
                            <span className="tabular-nums">{formatProformaMoney(Number(taxBreakdown.nhil))}</span>
                          </div>
                        )}
                        {taxBreakdown.getfund > 0 && (
                          <div className="flex justify-between text-sm text-slate-500">
                            <span>GETFund:</span>
                            <span className="tabular-nums">{formatProformaMoney(Number(taxBreakdown.getfund))}</span>
                          </div>
                        )}
                        {taxBreakdown.vat > 0 && (
                          <div className="flex justify-between text-sm text-slate-500">
                            <span>VAT:</span>
                            <span className="tabular-nums">{formatProformaMoney(Number(taxBreakdown.vat))}</span>
                          </div>
                        )}
                      </>
                    )}
                    <div className="flex justify-between text-sm pt-1 border-t border-slate-100">
                      ...
                      <span className="font-medium text-slate-800 tabular-nums">
                        {formatProformaMoney(Number(proforma.total_tax))}
                      </span>
                    </div>
                  </>
                )}
                <div className="flex justify-between text-lg pt-2 border-t-2 border-slate-800">
                  ...
                  <span className="font-bold text-slate-900 tabular-nums">
                    {formatProformaMoney(Number(proforma.total))}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-slate-800">
                  {proforma?.status === "draft"
                    ? "Send"
                    : `${proforma?.proforma_number ?? "Proforma"} — resend or share`}
                </h2>
              </div>
              ...
              {proforma?.status === "draft" && proforma?.customers?.name && (
                <div className="bg-slate-50 rounded-xl px-4 py-3 text-sm mb-2">
                  ...
                  <button
                    ...
                    disabled={sending || !(proforma?.customers?.whatsapp_phone?.trim() || proforma?.customers?.phone?.trim())}
                    title={!(proforma?.customers?.whatsapp_phone?.trim() || proforma?.customers?.phone?.trim()) ? "Customer phone number is missing." : undefined}
                  ...
                  >
                    ...
                    WhatsApp
                    {(proforma?.customers?.whatsapp_phone || proforma?.customers?.phone) && (
                  ...
                  <button
                    ...
                    disabled={sending || !proforma?.customers?.email?.trim()}
                    title={!proforma?.customers?.email?.trim() ? "Customer email is missing." : undefined}
