...
import { STOCK_OUT_REASONS } from "@/lib/service/materialMovementLabels"

export default function ServiceMaterialUseStockPage() {
  ...
  const [reason, setReason] = useState("used_for_job")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/service/materials/inventory/${id}/use-stock`, {
        method: "POST",
      ...
      if (!res.ok) throw new Error(typeof payload?.error === "string" ? payload.error : "Failed to use stock")
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-lg mx-auto px-4 py-8">
        <PageHeader title="Use stock" subtitle={materialName}
        ...
        <form onSubmit={handleSubmit} className="bg-white border rounded-xl shadow p-6 space-y-4">
          <div>
            ...
            <input type="number" step="0.01" min="0.01" max={onHand > 0 ? onHand : undefined} value={quantity}
              onChange={(e) => setQuantity(e.target.value)} required disabled={saving}
          ...
          <Button type="submit" disabled={saving} className="w-full">{saving ? "Saving…" : "Use stock"}</Button>
        </form>
      </div>
    </div>
  )
