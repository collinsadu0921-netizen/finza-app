...
function ServiceWithdrawalPageInner() {
  ...
  useEffect(() => {
    ...
    async function fetchCoa() {
      ...
      try {
        ...
        if (equity.length >= 1) setEquityId(equity[0].id)
        if (bankCash.length >= 1) setFromAccountId(bankCash[0].id)
      } catch (_) {
        // ignore
      } finally {
  ...
  const handleConfirmPost = async () => {
    ...
    try {
      const res = await fetch("/api/service/accounting/post-intent", {
        ...
        body: JSON.stringify({
          ...
          intent: {
            ...
            amount: numAmount,
            bank_or_cash_account_id: fromAccountId,
            equity_account_id: equityId,
            description: (description || "Owner Withdrawal").trim() || undefined,
          },
        }),
      })
      ...
      setError("Posted but no entry ID returned.")
    } catch (_) {
  ...
  if (!businessId) {
    return (
        ...
        <div className="p-6 max-w-2xl mx-auto">
          <p className="text-gray-600 dark:text-gray-400">No business found. You need an active business to record a withdrawal.</p>
          <button
            type="button"
  ...
  return (
      ...
      <div className="p-6 max-w-2xl mx-auto">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-6">
          Record money you take out of the business. This decreases your bank or cash balance and your equity.
        </p>
        ...
        {noBankCash && (
          <div className="mb-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 px-4 py-3 text-amber-800 dark:text-amber-200 text-sm">
            No bank or cash account found. Please create one in Chart of Accounts (with sub-type bank or cash).
          </div>
        )}
        {noEquity && (
          <div className="mb-4 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 px-4 py-3 text-amber-800 dark:text-amber-200 text-sm">
        ...
        <div className="space-y-4 mb-6">
          ...
          <div>
            <label className="flex items-center gap-1.5 text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              From (bank or cash)
              <span
                title="Only asset accounts with sub-type 'bank' or 'cash' appear here. Create or update accounts in Chart of Accounts."
        ...
        {showConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6">
              <p className="text-gray-900 dark:text-gray-100 font-medium mb-2">Confirm withdrawal</p>
              <p className="text-gray-600 dark:text-gray-400 text-sm mb-6">
                This will post to the ledger and cannot be edited. Continue?
              </p>
