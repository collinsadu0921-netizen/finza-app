...
export default function NewDraftPage() {
  ...
  const initializePage = async () => {
    ...
    try {
      ...
      if (!engagement) {
        setBlocked(true)
        setBlockedReason("An active engagement is required to create manual journal drafts.")
        setLoading(false)
        return
      }
  ...
  const loadPeriods = async (businessId: string) => {
    try {
      ...
      // Filter to only open periods for creating drafts
  ...
  const handleSave = async (submit: boolean = false) => {
    ...
    try {
      ...
      // If submitting, submit the draft
      if (submit && draftId) {
        const submitUrl =
          clientBusinessId
        ...
        if (!submitResponse.ok) {
          ...
          throw new Error(submitErrorData.message || "Draft created but failed to submit")
        }

        toast.showToast("Draft created and submitted successfully", "success")
        router.push(clientBusinessId ? `/accounting/journals?business_id=${clientBusinessId}` : "/accounting/journals")
      } else {
        toast.showToast("Draft saved successfully", "success")
        if (draftId && clientBusinessId) {
          router.push(`/accounting/journals/drafts/${draftId}?business_id=${clientBusinessId}`)
        } else {
          router.push(clientBusinessId ? `/accounting/journals?business_id=${clientBusinessId}` : "/accounting/journals")
        }
      }
      ...
      <ProtectedLayout>
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <PageHeader title="New Manual Journal Draft" />
  ...
  return (
    <ProtectedLayout>
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <PageHeader
            title="New Manual Journal Draft"
            subtitle={
              clientBusinessId ? (
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-400 text-blue-700 dark:text-blue-400 px-4 py-3 rounded mb-6">
            <p className="text-sm font-medium">
              <strong>Note:</strong> This is a draft. It will not affect the ledger until explicitly posted by a Partner.
            </p>
          </div>
