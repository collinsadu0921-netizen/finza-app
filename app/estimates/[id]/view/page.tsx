...
export default function EstimateViewPage() {
  ...
  const handleSend = async (action: "whatsapp" | "email" | "link") => {
    try {
      ...
      if (action === "email" && !estimate?.customers?.email?.trim()) {
        throw new Error("Customer email is missing.")
      }
      if (action === "whatsapp" && !(estimate?.customers?.whatsapp_phone?.trim() || estimate?.customers?.phone?.trim())) {
        ...
        {showSendModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl p-6 max-w-sm w-full">
              <h3 className="text-base font-semibold text-slate-900 mb-4">Send</h3>
              <div className="space-y-2.5">
                <button
                  type="button"
                  disabled={actionLoading || !(estimate?.customers?.whatsapp_phone?.trim() || estimate?.customers?.phone?.trim())}
                  onClick={() => void handleSend("whatsapp")}
                  title={!(estimate?.customers?.whatsapp_phone?.trim() || estimate?.customers?.phone?.trim()) ? "Customer phone number is missing." : undefined}
                  className="w-full bg-emerald-600 text-white px-4 py-3 rounded-xl hover:bg-emerald-700 disabled:opacity-60 flex items-center justify-center gap-2 text-sm font-medium transition-colors"
                >
                ...
                <button
                  type="button"
                  disabled={actionLoading || !estimate?.customers?.email?.trim()}
                  onClick={() => void handleSend("email")}
                  title={!estimate?.customers?.email?.trim() ? "Customer email is missing." : undefined}
                  className="w-full bg-slate-800 text-white px-4 py-3 rounded-xl hover:bg-slate-700 disabled:opacity-60 flex items-center justify-center gap-2 text-sm font-medium transition-colors"
                >
                ...
                <button
                  ...
                  className="w-full bg-white border border-slate-200 text-slate-700 px-4 py-3 rounded-xl hover:bg-slate-50 disabled:opacity-60 flex items-center justify-center gap-2 text-sm font-medium transition-colors"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
