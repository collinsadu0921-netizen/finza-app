...
export default function RetailInvitePage() {
  ...
  const nextPath = token ? `/retail/invite/${encodeURIComponent(token)}` : "/login"
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-lg flex-col justify-center px-4 py-10">
      ...
      {preview?.state === "revoked" && <p className="mt-3 text-sm">This invitation was revoked.</p>}
      ...
      {preview?.state === "ready" && preview.emailMatches === false && (
        <p className="mt-3 text-sm">This signed-in account does not match the invitation.</p>
      )}
      {preview?.state === "ready" && preview.emailMatches === true && (
        <div className="mt-4 space-y-3 text-sm">
          ...
          <button type="button" className="rounded-md bg-slate-900 px-3 py-2 text-white" disabled={working} onClick={() => void accept()}>
