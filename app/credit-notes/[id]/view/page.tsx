...
export default function CreditNoteViewPage() {
  ...
  const sendViaWhatsApp = () => {
    ...
    if (!phone) {
      toast.showToast("Customer phone number is missing.", "warning")
      return
    }
            ...
            <div className="flex flex-wrap gap-3">
              {(creditNote.status === "draft" || creditNote.status === "issued") && (
                <button
                  ...
                  title={!creditNote.invoices?.customers?.email?.trim() ? "Customer email is missing." : undefined}
                  className="inline-flex items-center gap-2 px-4 py-2 bg-slate-800 text-white text-sm font-semibold rounded-lg hover:bg-slate-700 disabled:opacity-50 transition-colors"
                >
              ...
              <button
                ...
                disabled={!(creditNote.invoices?.customers?.whatsapp_phone || creditNote.invoices?.customers?.phone)}
                title={!(creditNote.invoices?.customers?.whatsapp_phone || creditNote.invoices?.customers?.phone) ? "Customer phone number is missing." : undefined}
                className="inline-flex items-center gap-2 px-4 py-2 bg-[#25D366] text-white text-sm font-semibold rounded-lg hover:bg-[#1ebe5d] disabled:opacity-50 transition-colors"
              >
