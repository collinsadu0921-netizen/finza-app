  ...
  try {
    ...
    if (estimate.status !== "sent") {
      return NextResponse.json(
        { error: "Only sent quotes can be marked as declined" },
        { status: 400 }
      )
    }
    ...
    if (updateError || !updated) {
      console.error("Error declining quote (tenant):", updateError)
      return NextResponse.json(
        {
          success: false,
    ...
    console.error("Error declining quote (tenant):", error)
    const message = error instanceof Error ? error.message : "Internal server error"
    return NextResponse.json(
      {
        success: false,
        error: "Quote could not be declined. Please check your connection and try again.",
        message,
      },
      { status: 500 }
    )
