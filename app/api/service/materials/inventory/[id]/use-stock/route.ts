...
/**
 * POST /api/service/materials/inventory/[id]/use-stock
 * Tenant "Use stock" — decreases quantity. No COGS / ledger.
 */
export async function POST(
  ...
  try {
    ...
    const { quantity, reason, movement_date, note } = body as Record<string, unknown>
    ...
    if (!isValidStockOutReason(reasonCode)) {
    ...
    if (onHand < qty) {
      return NextResponse.json(
        { error: `Not enough stock. Quantity available: ${onHand}.` },
        { status: 400 }
      )
    }
    ...
    const movementType = stockOutMovementType(reasonCode)
    ...
    if (movErr) {
      console.error("use-stock movement failed:", movErr)
    ...
    console.error("POST use-stock:", err)
    return NextResponse.json(
