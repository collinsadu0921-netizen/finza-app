...
describe("GET /api/dashboard/service-timeline", () => {
  ...
  it("calls get_service_dashboard_timeline once with default periods=6", async () => {
    const rpc = jest.fn().mockResolvedValue({ data: timelineRows, error: null })
    mockCreateSupabase.mockResolvedValue({
      auth: { getUser: jest.fn().mockResolvedValue({ data: { user: { id: "u1" } } }) },
      rpc,
    } as any)
  ...
  it("returns empty timeline when RPC returns no rows", async () => {
    const rpc = jest.fn().mockResolvedValue({ data: [], error: null })
    mockCreateSupabase.mockResolvedValue({
      auth: { getUser: jest.fn().mockResolvedValue({ data: { user: { id: "u1" } } }) },
      rpc,
