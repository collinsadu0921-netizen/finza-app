...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!auth.authorized) {
      return NextResponse.json(
        { error: "Unauthorized. Only admins, owners, or accountants can view cash flow statement." },
        { status: 403 }
      )
    }
    ...
    const { data, error } = await getCashFlowReport(supabase, {
      businessId,
    ...
    console.error("Error in cash flow statement:", err)
    return NextResponse.json(
