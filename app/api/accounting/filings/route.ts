...
/**
 * GET /api/accounting/filings
 *
 * Firm-wide filing listing across all firm clients.
 * Returns filings scoped to the authenticated accountant's firm membership.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const periodFilter = searchParams.get("period")?.trim() ?? ""
    ...
    let query = supabase
      ...
      .select(`
        ...
        filed_at,
        created_at,
        businesses!client_filings_client_business_id_fkey (
          id,
          name
        )
      `)
    ...
    if (listErr) {
      console.error("firm-wide client_filings list error:", listErr)
