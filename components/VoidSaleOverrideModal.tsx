...
interface VoidSaleOverrideModalProps {
  isOpen: boolean
  onClose: () => void
  saleId: string
  cashierId: string
...
export default function VoidSaleOverrideModal({
  isOpen,
  onClose,
  saleId,
  cashierId,
  onSuccess,
}: VoidSaleOverrideModalProps) {
      ...
      <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4 shadow-xl my-8 relative">
        ...
        <div className="bg-red-50 border-2 border-red-300 rounded p-4 mb-4">
          <p className="text-sm text-gray-700 mb-2">
            Supervisor approval is required to void this sale.
          </p>
          <p className="text-xs text-gray-600">
            This action cannot be undone and will be recorded in the system.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 pt-4">
            ...
            >
              {loading ? "Verifying..." : "Approve Void"}
            </button>
          </div>
        </form>
      </div>
