-- ============================================================================
-- Remove temporary diagnostics (p_debug and NOTICEs) from trial balance path
-- ============================================================================
-- Restore get_trial_balance_from_snapshot(p_period_id) and
-- generate_trial_balance(p_period_id, p_generated_by) without debug params.
-- Table references use public.trial_balance_snapshots for consistency with 239.
-- Drop the (UUID, boolean) / (UUID, UUID, boolean) overloads from 240 so
-- get_trial_balance_from_snapshot(uuid) and generate_trial_balance(uuid, uuid)
-- are unique.
-- ============================================================================

DROP FUNCTION IF EXISTS get_trial_balance_from_snapshot(UUID, boolean);
DROP FUNCTION IF EXISTS generate_trial_balance(UUID, UUID, boolean);
...
COMMENT ON FUNCTION generate_trial_balance(UUID, UUID) IS 'PHASE 9: Canonical trial balance generator. Ledger-only source (period_opening_balances + journal_entry_lines).';
COMMENT ON FUNCTION get_trial_balance_from_snapshot(UUID) IS 'PHASE 9: Returns trial balance from canonical snapshot. If snapshot doesn''t exist, generates it first.';
