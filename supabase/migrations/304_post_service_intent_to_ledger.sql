-- ============================================================================
-- Service intent posting: engine-controlled debit/credit (no user-chosen sides)
-- ============================================================================
-- post_service_intent_to_ledger(p_business_id, p_user_id, p_entry_date, p_intent jsonb)
-- - Owner-only (business.owner_id = p_user_id)
-- - Period must exist and not be locked
-- - Lines derived from intent_type (OWNER_CONTRIBUTION: DR bank CR equity; OWNER_WITHDRAWAL: DR equity CR bank)
-- - Single INSERT for journal_entry_lines so trigger_enforce_double_entry_balance sees balanced entry
-- ============================================================================

-- Allow source_type = 'service_intent' on journal_entries
ALTER TABLE journal_entries
  DROP CONSTRAINT IF EXISTS journal_entries_source_type_check;
ALTER TABLE journal_entries
  ADD CONSTRAINT journal_entries_source_type_check
  CHECK (source_type IS NULL OR source_type IN ('proposal', 'adjustment', 'manual_draft', 'opening_balance', 'service_intent'));
...
COMMENT ON FUNCTION post_service_intent_to_ledger(UUID, UUID, DATE, JSONB) IS
