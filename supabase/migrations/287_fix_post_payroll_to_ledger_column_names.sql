-- Fix post_payroll_to_ledger: use actual payroll_runs column names.
-- Table has total_gross_salary and total_net_salary (not total_gross / total_net).
-- No schema changes; only correct SELECT column reference mismatch.
...
BEGIN
  -- Get payroll run details (use actual payroll_runs column names)
  SELECT 
    business_id,
    payroll_month,
    total_gross_salary,
  ...
  -- Canonical rule: period must be open for posting
  ...
  -- Insert ALL lines in a SINGLE statement so statement-level balance trigger sees balanced entry (canonical rule)
  INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit, description)
  VALUES
    (v_journal_entry_id, v_payroll_expense_account_id, v_total_gross + v_total_allowances, 0, 'Gross Salaries and Allowances'),
...
COMMENT ON FUNCTION post_payroll_to_ledger IS 
  'Posts payroll to ledger with EXPLICIT posting_source = ''system''. Reads total_gross_salary and total_net_salary from payroll_runs. Enforces assert_accounting_period_is_open. Inserts all journal lines in one statement for balance trigger.';
