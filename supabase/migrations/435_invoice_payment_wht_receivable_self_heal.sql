-- ============================================================================
-- Migration 435: WHT receivable self-heal on invoice payment posting
-- ============================================================================
-- post_invoice_payment_to_ledger (434) requires chart_of_accounts_control_map
-- WHT_RECEIVABLE. Businesses created before 434, or with 2155 in accounts but
-- never synced to chart_of_accounts, hit a hard error when recording WHT.
-- This mirrors 434's idempotent sync at payment time for account 2155 only.
-- ============================================================================
...
BEGIN
  ...
  IF v_wht_amount > 0 THEN
    ...
    SELECT business_id_val, 'WHT_RECEIVABLE', '2155'
    WHERE EXISTS (
      SELECT 1 FROM chart_of_accounts c
      WHERE c.business_id = business_id_val
        AND c.account_code = '2155'
...
'Posts invoice payment to ledger. WHT receivable uses WHT_RECEIVABLE control map (default 2155); syncs chart + mapping from accounts when missing. Paystack clears through BANK.';
