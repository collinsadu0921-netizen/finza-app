-- ============================================================================
-- Allow source_type = 'service_intent' without source_id in proposal gating
-- ============================================================================
-- post_service_intent_to_ledger inserts journal_entries with source_type =
-- 'service_intent' and no external source record, so source_id is NULL.
-- validate_proposal_gating currently requires "source_type and source_id must
-- both be provided or both be NULL". This migration adds service_intent to the
-- list of source types that skip proposal gating (like manual_draft and
-- opening_balance), so service_intent with source_id NULL is allowed.
-- ============================================================================
...
BEGIN
  -- manual_draft, opening_balance, and service_intent skip proposal gating; no source_id required
  IF p_source_type IN ('manual_draft', 'opening_balance', 'service_intent') THEN
    RETURN TRUE;
  END IF;
...
'Validates proposal gating for journal entry insert. manual_draft, opening_balance, and service_intent skip gating; proposal and adjustment are validated as before.';
