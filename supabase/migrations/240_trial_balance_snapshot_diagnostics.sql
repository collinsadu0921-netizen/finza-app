-- ============================================================================
-- TEMPORARY: Diagnostics for trial_balance_snapshots permission (p_debug gate)
-- ============================================================================
-- Enable with get_trial_balance_from_snapshot(period_id, true) to see
-- current_user, session_user, auth.uid(), snapshot_exists. Remove after
-- confirmation (see 241_trial_balance_snapshot_diagnostics_remove.sql).
-- ============================================================================

-- generate_trial_balance: add p_debug, NOTICEs when true (before/after INSERT)
...
BEGIN
  IF p_debug THEN
    RAISE NOTICE 'diag generate_trial_balance current_user=% session_user=% uid=%', current_user, session_user, auth.uid();
  END IF;
  ...
  IF p_debug THEN
    RAISE NOTICE 'diag calling generate_trial_balance INSERT/UPDATE';
  END IF;
  ...
  IF p_debug THEN
    RAISE NOTICE 'diag generate_trial_balance finished';
  END IF;
...
-- get_trial_balance_from_snapshot: add p_debug, NOTICEs when true, pass p_debug to generate_trial_balance
...
DECLARE
  snapshot_record public.trial_balance_snapshots;
  account_data JSONB;
BEGIN
  IF p_debug THEN
    RAISE NOTICE 'diag get_trial_balance_from_snapshot current_user=% session_user=% uid=%', current_user, session_user, auth.uid();
  END IF;
  ...
  IF p_debug THEN
    RAISE NOTICE 'diag snapshot_exists=%', (snapshot_record.id IS NOT NULL);
  END IF;

  IF NOT FOUND THEN
    IF p_debug THEN
      RAISE NOTICE 'diag calling generate_trial_balance';
    END IF;
    PERFORM generate_trial_balance(p_period_id, NULL, p_debug);
    IF p_debug THEN
      RAISE NOTICE 'diag generate_trial_balance finished';
    END IF;
...
COMMENT ON FUNCTION generate_trial_balance(UUID, UUID, boolean) IS 'PHASE 9: Canonical trial balance generator. p_debug: temporary diagnostics (remove in 241).';
COMMENT ON FUNCTION get_trial_balance_from_snapshot(UUID, boolean) IS 'PHASE 9: Returns trial balance from snapshot. p_debug: temporary diagnostics (remove in 241).';
