-- ============================================================================
-- Enforce: Bill payment cannot post unless the bill is OPEN and posted.
-- ============================================================================
-- 1) Status guard: bill must be 'open' (no payment for draft/paid/etc.).
-- 2) Dependency guard: bill liability JE must exist (auto-post if missing).
-- Ledger immutable; post_journal_entry unchanged; minimal diff.
-- ============================================================================
...
BEGIN
  ...
  -- Get bill details (id, status, bill_number for guards and description)
  ...
  -- Status guard: cannot post payment for non-open bill (e.g. draft)
...
COMMENT ON FUNCTION post_bill_payment_to_ledger(UUID) IS
  'Posts bill payment to ledger. Bill must be open and posted first. Ensures bill JE exists (calls post_bill_to_ledger if missing). posting_source = system.';
