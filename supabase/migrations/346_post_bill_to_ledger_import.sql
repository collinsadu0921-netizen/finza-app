-- ============================================================================
-- Migration 346: post_bill_to_ledger — Import Bill Support
-- ============================================================================
-- Extends post_bill_to_ledger to handle import bills (bill_type = 'import').
--
-- Standard bill journal (unchanged):
--   Dr  5200 Expense         = subtotal
--   Dr  2100/2110/2120 Input Tax = tax amounts
--   Cr  2000 AP              = total (or total - WHT)
--   Cr  2150 WHT Payable     = WHT amount (if applicable)
--
-- Import bill journal:
--   Dr  landed_cost_account  = cif + duty + levies  (1200/1210/5000/5200/5210)
--   Dr  5220 Clearing Fee    = clearing_agent_fee    (if > 0)
--   Dr  2100/2110/2120 Input Tax = VAT/NHIL/GETFund  (claimable)
--   Cr  2000 AP              = total - WHT
--   Cr  2150 WHT Payable     = WHT (if applicable)
-- ============================================================================
...
DECLARE
  ...
  -- Import-specific
  v_bill_type          TEXT;
  v_landed_cost        NUMERIC;  -- CIF + duty + levies
...
BEGIN
  ...
  IF v_bill_type = 'import' THEN
    ...
    -- Dr Landed cost account (CIF + duty + port levies)
    ...
    -- Dr Clearing agent fee (5220) if applicable
    IF v_clearing_fee > 0 THEN
...
COMMENT ON FUNCTION post_bill_to_ledger(UUID, TEXT, TEXT, TEXT) IS
  'Posts bill to ledger. Standard: Dr Expense + Dr Input Tax / Cr AP (± WHT). '
  'Import: Dr Landed Cost (CIF+duty+levies) + Dr Clearing Fee + Dr Input Tax / Cr AP (± WHT). Balanced.';
