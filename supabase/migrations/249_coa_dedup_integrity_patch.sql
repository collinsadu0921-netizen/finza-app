-- ============================================================================
-- Migration 249: CoA Dedup Integrity Patch
-- ============================================================================
-- Fixes remaining structural risks from 248 without changing business behavior:
-- 1. Dynamic FK discovery — find ALL FKs referencing accounts(id); reassign every one.
-- 2. Snapshot stale marking — capture affected businesses before any cleanup; use for idempotent reruns.
-- 3. Canonical selection stability — COALESCE created_at and jel count.
-- 4. Index creation safety — LOCK TABLE accounts before creating partial unique index.
-- 5. Dynamic orphan validation — check every discovered FK column for orphans.
--
-- Safe to run after 248 (idempotent). No posting/snapshot schema/RLS/control map changes.
-- ============================================================================
...
-- (period_opening_balances and period_account_snapshot already handled with merge logic above)
-- ============================================================================
DO $$
DECLARE
  r RECORD;
...
BEGIN
  ...
  LOOP
    q := format(
      'UPDATE %I.%I t SET %I = c.canonical_account_id '
      'FROM _canonical_accounts c, unnest(c.duplicate_account_ids) AS dup_id(uid) '
      'JOIN accounts dup_account ON dup_account.id = dup_id.uid '
      'JOIN accounts canonical_account ON canonical_account.id = c.canonical_account_id '
      'WHERE t.%I = dup_id.uid AND dup_account.business_id = canonical_account.business_id',
...
BEGIN
  ...
  LOOP
    ...
    IF v_orphan_count > 0 THEN
      RAISE EXCEPTION 'Orphan FKs: %.%.% has % rows with account_id not in accounts', r.schema_name, r.table_name, r.column_name, v_orphan_count;
    END IF;
  END LOOP;
...
COMMENT ON INDEX accounts_unique_business_code_active_idx IS
  'One active account per (business_id, code). Partial index WHERE deleted_at IS NULL. Integrity patch 249.';
