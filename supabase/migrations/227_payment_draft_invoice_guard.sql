-- ============================================================================
-- Migration: No ledger posting for payments on draft invoices
-- ============================================================================
-- Invariant: Payments must not post to the ledger if the linked invoice is
-- in draft status. Draft invoices are non-existent to accounting.
--
-- Enforcement: In both payment posting functions, load invoice status and
-- RAISE if draft. Guarantees no JE is created even if API is bypassed or
-- payment is inserted directly.
-- ============================================================================
...
COMMENT ON FUNCTION post_invoice_payment_to_ledger(UUID) IS
  'Post payment to ledger (Dr Cash/Bank, Cr AR). Raises if linked invoice is draft. No JE for draft invoices.';
COMMENT ON FUNCTION post_payment_to_ledger(UUID, TEXT, TEXT, TEXT) IS
  'Alias/canonical payment posting. Raises if linked invoice is draft. Issue the invoice first.';
