-- ============================================================================
-- Payment posting idempotency: advisory lock + re-check (match invoice/expense).
-- One payment → one journal entry. No schema or contract changes.
-- ============================================================================
...
BEGIN
  ...
  SELECT invoice_number, id, status INTO invoice_record FROM invoices WHERE id = payment_record.invoice_id;
  IF NOT FOUND THEN
  ...
  -- Serialize concurrent posting for the same payment (exactly-once under concurrency)
  PERFORM pg_advisory_xact_lock(hashtext(business_id_val::text), hashtext(p_payment_id::text));

  -- IDEMPOTENCY: Re-check after lock; return existing JE if already posted
...
COMMENT ON FUNCTION post_invoice_payment_to_ledger(UUID) IS
  'Post payment to ledger (Dr Cash/Bank, Cr AR). Idempotent: advisory lock + re-check. Raises if linked invoice is draft.';
COMMENT ON FUNCTION post_payment_to_ledger(UUID, TEXT, TEXT, TEXT) IS
  'Canonical payment posting. Idempotent: advisory lock + re-check. Raises if linked invoice is draft.';
