-- ============================================================================
-- Migration 468: Correct post_payroll_to_ledger — pension split journal balance
-- ============================================================================
-- Deploy this after migration 467 (do not rewrite 467 on databases where it ran).
--
-- Root cause remediated:
--   Migration 467 used multiple INSERTs into journal_entry_lines. The DB uses a
--   statement-level balance trigger; after the first INSERT the entry was missing
--   Cr 2231 / Cr 2232, raising "Journal entry is not balanced" with
--   Difference equal to total employee + employer pension.
--
-- Fix:
--   One INSERT bundles base lines + optional tier credits + salary advance clearing.
--
-- Pension split behaviour:
--   total_pension = COALESCE(total_ssnit_employee,0) + COALESCE(total_ssnit_employer,0)
--   Snapshots: SUM(COALESCE(tier1_ssnit_remittance,0)), same for tier2.
--   If total_pension <= 0.01 → no pension lines.
--   Else if non-negative snapshots, snap sum > 0.01, and within 0.02 of total → use them.
--   Else fallback: tier1 = ROUND(total * 13.5/18.5, 2), tier2 = ROUND(total - tier1, 2).
--
-- Example (balanced journal shape):
--   total_ssnit_employee = 313.50
--   total_ssnit_employer = 741.00
--   → total_pension = 1054.50
--   Fallback: tier1 = 769.50, tier2 = 285.00
-- ============================================================================
...
'Posts payroll run to ledger (SECURITY DEFINER). Single INSERT into journal_entry_lines (balance trigger). Balanced: DR 5600+5610 (+ advance Dr 2241); CR 2230+2231+2232+2240+2241 (+ advance Cr 1110). Pension: CR 2231 Tier 1, CR 2232 Tier 2. Idempotent: returns existing payroll_runs.journal_entry_id if set.';
