-- Fix volatility: run_period_close_checks calls get_trial_balance_from_snapshot,
-- which may INSERT/UPDATE trial_balance_snapshots via generate_trial_balance.
-- STABLE caused "cannot execute INSERT in a read-only transaction" when the
-- RPC was run in a read-only context. Recreate as VOLATILE.

DROP FUNCTION IF EXISTS run_period_close_checks(UUID, UUID);
...
COMMENT ON FUNCTION run_period_close_checks IS
  'Pre-close audit checks: (1) Trial balance balanced, (2) Period AR matches operational or resolved, (3) No unposted WARN/FAIL mismatches. Returns ok and structured failures. VOLATILE: may trigger get_trial_balance_from_snapshot which can INSERT/UPDATE trial_balance_snapshots.';
