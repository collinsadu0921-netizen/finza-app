-- ============================================================================
-- MIGRATION: Insert journal_entry_lines in single statement (double-entry trigger)
-- ============================================================================
-- trigger_enforce_double_entry_balance is FOR EACH STATEMENT. Loop-based
-- INSERT produced one statement per line; trigger ran after first line only
-- and raised "Journal entry is not balanced". Replace with single
-- INSERT...SELECT so trigger runs once after all lines. No other logic changed.
-- ============================================================================
...
BEGIN
  ...
  -- Single INSERT so trigger_enforce_double_entry_balance (FOR EACH STATEMENT) sees all lines
...
'Idempotent post manual journal draft to ledger. Supports owner-mode (accounting_firm_id IS NULL) and firm-mode. Returns existing entry if already posted. Inserts all lines in one statement for double-entry trigger.';
